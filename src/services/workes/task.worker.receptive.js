...
export async function startTaskWorkerReceptive() {
    ...
    channel.consume(queue, async (msg) => {
        ...
        const task = body.bodyTask;
        const repostaEnviada = body.resposta;
        try {
            ...
            if (dadosDaMesagen.value.messages) {
                ...
                if (idMensagem && numeroDoContato) {
                    ...
                    if (usuario.user) {
                        const dadosUser = usuario.user;
                        await criarHistoricoDeConversa(dadosUser.id, tipoDaMensagem, mensagemRecebida, respostaParaMensagem, String(new Date(Number(timesTampMensagem) * 1000)), "");
                    }
                }
