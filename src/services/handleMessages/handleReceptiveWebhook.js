...
export async function HandleReceptiveWebhook(task) {
    try {
        ...
        if (dadosDaMesagen.value.messages) {
            ...
            if (idMensagem && numeroDoContato) {
                ...
                if (tipoDaMensagem === "audio") {
                    mensagem = await tratarMensagensDeAudio(dadosDoBodyDaMensagem, idMensagem, numeroDoContato, MENSAGM_DEFAULT);
                }
                else if (tipoDaMensagem === "text") {
                    mensagem = await tratarMensagensDeTexto(dadosDoBodyDaMensagem, idMensagem, numeroDoContato, MENSAGM_DEFAULT);
                }
                else {
...
async function tratarMensagensDeAudio(dados, idMensagem, numeroDoContato, MENSAGM_DEFAULT) {
    try {
        const urlAudio = dados.audio?.url;
        const idAudio = dados.audio?.id;
        let mensagem;
        if (urlAudio && idAudio) {
            const resultgGetAudio = await getAudio(idAudio, MENSAGM_DEFAULT);
...
async function tratarMensagensDeTexto(dados, idMensagem, numeroDoContato, MENSAGM_DEFAULT) {
    try {
        let responseToUser;
        if (dados.text?.body) {
...
async function sendBodyToMenssage(idMensagem, numeroDoContato, consultaResposta) {
    try {
...
async function splitText(text, limit = 3800) {
    const parts = [];
    let current = "";
